...
<div class="p-6" x-data="staffApplicationsIndex()" x-init="init()">
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            ...
            <h1 class="text-2xl font-bold mt-2">My encoded applications</h1>
        </div>
        <a href="/staff/encode" class="btn btn-primary btn-sm">Encode applicant</a>
    </div>
    ...
    <template x-if="!loading && !error && applications.length === 0">
        <div class="card bg-base-100 shadow">
            <div class="card-body">
                <p class="text-base-content/70">No applications yet.</p>
                <p class="text-sm text-base-content/60">Applications you encode will appear here.</p>
                <a href="/staff/encode" class="btn btn-primary btn-sm w-fit mt-2">Encode applicant</a>
            </div>
        </div>
    </template>
                ...
                <tbody>
                    <template x-for="a in applications" :key="a.id">
                        <tr>
                            ...
                            <td>
                                <a :href="'/staff/applications/' + a.id" class="btn btn-ghost btn-sm">View</a>
                                <a x-show="a.status === 'revision_requested'" :href="'/staff/applications/' + a.id + '/revise'" class="btn btn-warning btn-sm ml-1">Revise</a>
                            </td>
                        </tr>
                    </template>
                </tbody>
...
<script>
function staffApplicationsIndex() {
    return {
        applications: [],
        filterStatus: '',
        loading: true,
